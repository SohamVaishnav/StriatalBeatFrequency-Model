...
group1 = NeuronGroup(10, eqs_cortical, threshold='v > cortical_v_threshold',
                     reset='v = cortical_v_reset', method='euler', refractory=10*ms)
group2 = NeuronGroup(10, eqs_cortical, threshold='v > cortical_v_threshold',
                     reset='v = cortical_v_reset', method='euler', refractory=10*ms)
...
syn1 = Synapses(group1, group3, on_pre='v_post += 0.85 * mV')
syn2 = Synapses(group2, group3, on_pre='v_post += 0.85 * mV')
syn1.connect(p=0.5)
syn2.connect(p=0.5)
...
plt.subplot(311)
plt.plot(spike_monitor1.t / ms, spike_monitor1.i, '.b', label='Cortical (Group 1: Delta)')
...
input1 = 15 * np.sin(2 * np.pi * 2.5 * time)
input2 = 15 * np.sin(2 * np.pi * 6 * time)
plt.subplot(313)
plt.plot(time, input1, label='Delta Waves (2.5 Hz)')
