...
eqs_striatal = '''
dv/dt = (v_rest - v + I) / tau_striatal : volt (unless refractory)
I = 0 * volt : volt  # No intrinsic oscillatory input, relies on input from neurons1
'''
...
neurons_striatal = NeuronGroup(num_neurons, eqs_striatal, threshold='v > v_threshold_striatal',
...
# plt.subplot(313)
# plot(time, 25*np.sin(2*np.pi*14*time), label='Group 1')
# plot(time, 30*np.sin(2*np.pi*24*time), label='Group 2')
# xlabel('Time (ms)')
